...
@AllArgsConstructor
@Slf4j
public class ArtefactUploadRequest implements Function<InputStream, APIGatewayV2HTTPResponse> {
	...
	public APIGatewayV2HTTPResponse apply(InputStream in) {
		...
		try {
			...
			try {
				...
				return ApiGatewayResponseUtil.getAPIGatewayV2HTTPResponse(HttpResponseConstant.STATUS_CODE_BAD_REQUEST,
			...
			String key = "Aws-" + DateUtils.getCurrentDateShortStr() + "/" + artefactId + "/" + filename;
			...
			if (StringUtils.isNotBlank(artefact.getMirisDocId()) && artefactService
				.hasFileWithSameDocId(artefact.getMirisDocId(), artefact.getArtefactClassType())) {
				log.info("artefactService.hasFileWithSameDocId returns true");
				item.setStatus(ArtefactStatus.INDEXED.toString());
				log.info("set current artefact as indexed with mirisDocId: {}", artefact.getMirisDocId());
			}
			else {
				log.info(
						"artefactService.hasFileWithSameDocId returns false meaning no existing artefact with mirisDocId: {}",
			...
			log.info("saving transaction|correlation-id: {} on path", item.getArtefactItemId());
