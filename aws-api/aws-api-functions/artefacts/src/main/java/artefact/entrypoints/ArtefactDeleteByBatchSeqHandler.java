...
public class ArtefactDeleteByBatchSeqHandler extends CoreRequestHandler
        implements RequestHandler<APIGatewayV2HTTPEvent, APIGatewayV2HTTPResponse> {

    private static final Logger logger = LoggerFactory.getLogger(ArtefactDeleteByBatchSeqHandler.class);
    ...
    public APIGatewayV2HTTPResponse handleRequest(APIGatewayV2HTTPEvent event, Context context) {
        ...
        try{
            ...
            if(ArtefactStatus.DELETED.getStatus().equalsIgnoreCase(batchDtl.getStatus())){
                return buildResponse(logger,SC_BAD_REQUEST,new ApiMapResponse(Map.of(KEY_MESSAGE, "Batch is already in DELETED")));
            }
            // soft deleting the batch
            batchService.updateBatchWithStatus(batchSeq,ArtefactStatus.DELETED.toString());

            return buildResponse(logger,SC_OK,new ApiMapResponse(Map.of(KEY_MESSAGE,"Batch deleted successfully")));
        } catch (IOException e) {
