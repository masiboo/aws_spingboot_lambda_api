...
public class ArtefactLockByBatchSeqHandler extends CoreRequestHandler
        implements RequestHandler<APIGatewayV2HTTPEvent, APIGatewayV2HTTPResponse> {

    private static final Logger logger = LoggerFactory.getLogger(ArtefactLockByBatchSeqHandler.class);
    @Override
    public APIGatewayV2HTTPResponse handleRequest(APIGatewayV2HTTPEvent event, Context context) {
        ...
        try{
            ...
            if(batchDtl.isLocked()){
                return buildResponse(logger,SC_BAD_REQUEST,new ApiMapResponse(Map.of(KEY_MESSAGE, "Batch is already in LOCKED")));
            }

            batchService.updateLockState(batchSeq,true);

            return buildResponse(logger,SC_OK,new ApiMapResponse(Map.of(KEY_MESSAGE,"Batch locked successfully")));
        } catch (IOException e) {
