...
if [ -z "$PROFILE_NAME" ]
then
  echo "Profile name is null"
  npx aws-cdk deploy *AwsCfnPreRequisiteStack --require-approval never
else
  npx aws-cdk deploy *AwsCfnPreRequisiteStack --require-approval never --profile $PROFILE_NAME
  echo ......
fi
echo .
echo .